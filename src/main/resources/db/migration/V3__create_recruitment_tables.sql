-- ============================================================
-- V3: 모집 테이블 (recruitments, applications)
-- ============================================================
...
(
    ...
    activity_city     VARCHAR(100) NOT NULL COMMENT '주요 활동 광역지방자치단체',
    activity_district VARCHAR(100) NOT NULL COMMENT '주요 활동 기초지방자치단체',
    activity_days     VARCHAR(100) NOT NULL COMMENT '주요 활동 요일 (콤마 구분)',
...
) COLLATE = utf8mb4_unicode_ci COMMENT = '모집 공고';
...
(
    id                      CHAR(128)    NOT NULL COMMENT '지원서 식별키' PRIMARY KEY,
    ...
    updated_at              DATETIME(6)  NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6) COMMENT '변경 시각',
    deleted_at              DATETIME(6)  NULL COMMENT '삭제 시각'
) COLLATE = utf8mb4_unicode_ci COMMENT = '입단 지원서';

CREATE INDEX idx_applications_recruitment ON applications (recruitment_id, status, deleted_at);
CREATE INDEX idx_applications_club ON applications (club_id, status, deleted_at);
CREATE INDEX idx_applications_applicant ON applications (applicant_user_id, deleted_at);
