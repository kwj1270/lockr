...
import com.official.lockr.domain.club.sqaud.domain.board.Formation;
import com.official.lockr.domain.club.sqaud.domain.board.Lineup;
import com.official.lockr.domain.club.sqaud.domain.board.TacticalBoard;
import com.official.lockr.domain.club.sqaud.domain.board.TacticalBoardRepository;
...
public class JOOQTacticalBoardRepository implements TacticalBoardRepository {
    ...
    @Override
    public List<TacticalBoard> findAllBySquadId(final String squadId) {
        final List<TacticalBoardsEntity> entities = tacticalBoardsDao.ctx()
                .selectFrom(TACTICAL_BOARDS)
                .where(TACTICAL_BOARDS.SQUAD_ID.eq(squadId))
                .fetchInto(TacticalBoardsEntity.class);
    ...
    private void upsertEntry(final TacticalBoard tacticalBoard) {
        tacticalBoardsDao.ctx()
                ...
                .set(TACTICAL_BOARDS.SQUAD_ID, tacticalBoard.getSquadId())
    ...
    private void syncPlayers(final TacticalBoard tacticalBoard) {
        ...
        final Lineup lineup = tacticalBoard.getLineup();
        ...
        // 2. 새로운 플레이어들 삽입
        insertFieldPlayers(tacticalBoardId, lineup.getFieldPlayers().getFiledPlayers());
        insertBenchPlayers(tacticalBoardId, lineup.getBenchPlayers().getBenchPlayers());
        insertNoneSelectedPlayers(tacticalBoardId, lineup.getNoneSelectedPlayers().getNoneSelectedPlayers());

        // 3. 포메이션 삽입
        insertFormation(tacticalBoardId, lineup.getFormation());
    }
    ...
    private Formation findFormation(final String tacticalBoardId) {
        ...
        return Formation.valueOf(formationsEntity.getFormation());
    }
    ...
    private void insertFormation(final String tacticalBoardId, final Formation formation) {
        ...
        formationsDao.ctx()
                ...
                .set(FORMATIONS.FORMATION, formation.name())
                .execute();
    }
}
